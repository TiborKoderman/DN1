...
if [[ -e $testedscript ]]; then 
    ...
    output=$("$testedscript" "$testroot/imenik" "py" "torch" 2>/dev/null) || { test $? -eq 126 && echo "Nimate pravice zagnati skipte." && exit; }
...
if [[ -e $testedscript ]]; then 
   ...
   output=$("$testedscript" imenik1 imenik2 2>/dev/null) 
